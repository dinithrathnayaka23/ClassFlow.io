...
export function Users({ filter }: { filter?: string }) {
  ...
  const load = () =>
    api<Page<UserRow>>(
      `/users?size=100${filter ? `&role=${filter}` : ""}`,
  ...
  return (
    <>
      <SectionTitle
        ...
        title={
          filter ? `${filter[0]}${filter.slice(1).toLowerCase()}s` : "People"
        }
        action={
      ...
      {users.length ? (
              ...
              <tbody>
                {users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={user.active ? "text-neon" : "text-red-300"}
                      >
        ...
        <Empty title="No users found" text="Add a user to get started." />
      )}
      <Modal
